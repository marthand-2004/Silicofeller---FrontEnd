  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "SilicoFeller — AI Quantum Chip Design" },
      {
        ...
        content:
          "AI-powered quantum chip design. From prompt to fabricated qubit array.",
      },
    ],
  }),
  ...
  return (
    <main className="relative min-h-screen bg-background text-foreground">
      ...
      >
        ...
        />
        <div aria-hidden className="pointer-events-none absolute inset-x-0 bottom-0 h-40 bg-gradient-to-b from-transparent to-[#0A0A0F]" />
        ...
        >
          ...
          <p className="mt-6 max-w-xl text-[1.0625rem] leading-relaxed text-foreground/70">
            SilicoFeller transforms natural-language prompts into production-ready
            quantum chip architectures — transmon arrays, error-correction layouts
            and superconducting qubit topologies, generated in seconds.
          </p>

          <div className="mt-8 flex flex-wrap items-center gap-3">
            <Button asChild className="h-12 rounded-full bg-foreground px-6 text-sm font-semibold text-background hover:bg-foreground/90">
              <Link to="/designer">
          ...
          <div className="mt-12 flex flex-wrap items-center gap-x-8 gap-y-3 text-xs text-foreground/55">
            <span className="flex items-center gap-1.5"><Shield className="h-3.5 w-3.5" /> SOC 2 ready</span>
            <span className="flex items-center gap-1.5"><Zap className="h-3.5 w-3.5" /> Generates in seconds</span>
            <span className="flex items-center gap-1.5"><Cpu className="h-3.5 w-3.5" /> 128+ qubit topologies</span>
          </div>
          </motion.div>
      ...
      <Section
        id="about"
        eyebrow="What we do"
        title="From a prompt to a fabricated quantum chip — automatically."
        tone="paper"
      >
        <p className="max-w-3xl text-[0.9375rem] leading-relaxed text-muted-foreground">
          SilicoFeller uses advanced AI models to translate natural-language
          engineering requirements into quantum chip designs — qubit layouts,
          coupling maps, readout networks and control HDL. Describe what you
          need in plain English; we generate the architecture.
        </p>
      ...
      <Section
        id="technology"
        eyebrow="Technology"
        title="Built on quantum-aware AI."
        tone="grid"
      >
      ...
      <Section id="features" eyebrow="Features" title="Everything you need to ship quantum silicon faster." tone="dark">
      ...
      <Section id="demo" eyebrow="Live demo" title="A prompt in. A quantum chip out." tone="paper">
        <div className="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-2">
          <div className="rounded-2xl border border-border bg-foreground p-5 font-mono text-sm text-background" style={{ boxShadow: "var(--shadow-card)" }}>
      ...
      {/* ───────── BLOG — elevated cards ───────── */}
      <Section id="blog" eyebrow="Research & insights" title="From the SilicoFeller blog." tone="elevated">
        <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {POSTS.map((p) => (
            ...
            >
              ...
              <p className="mt-3 text-[11px] font-medium uppercase tracking-wider text-[#F26B3A]">{p.tag}</p>
        ...
        <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-[1.2fr_1fr]">
          ...
          >
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <FormInput label="Company" placeholder="Acme Semiconductors" className="sm:col-span-2" />
            ...
            <Button type="submit" className="mt-5 h-11 rounded-full bg-white px-6 text-sm font-semibold text-black hover:bg-white/90">
              Send message <ArrowRight className="ml-1.5 h-4 w-4" />
            </Button>
          </form>
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-xl">
            <p className="text-sm text-white/70">
              Talk to our quantum engineering team about pilots, integrations,
              and enterprise deployments.
            </p>
            <div className="mt-5 flex items-center gap-3">
              <a className="grid h-10 w-10 place-items-center rounded-full border border-white/15 text-white transition-colors hover:bg-white/10" href="#" aria-label="LinkedIn"><Linkedin className="h-4 w-4" /></a>
              <a className="grid h-10 w-10 place-items-center rounded-full border border-white/15 text-white transition-colors hover:bg-white/10" href="#" aria-label="GitHub"><Github className="h-4 w-4" /></a>
              <a className="grid h-10 w-10 place-items-center rounded-full border border-white/15 text-white transition-colors hover:bg-white/10" href="mailto:hello@silicofeller.com" aria-label="Email"><Mail className="h-4 w-4" /></a>
            </div>
          </div>
        </div>
      </Section>
      ...
      <footer className="relative z-10 border-t border-white/10 bg-[#050507] px-6 py-10 text-white lg:px-10">
        ...
        <div className="mx-auto mt-8 flex max-w-6xl flex-col items-center justify-between gap-2 border-t border-white/10 pt-6 text-xs text-white/50 sm:flex-row">
          ...
          {user && (
            <p>
              Signed in as <span className="font-medium text-white">{user.name}</span> · {ROLE_LABEL[user.role]}
            </p>
          )}
        </div>
      </footer>
...
function SiteNav({ scrolled, user }: { scrolled: boolean; user: ReturnType<typeof useAuth>["user"] }) {
  return (
    <header
    ...
    >
      <div className="flex items-center justify-between px-6 py-4 lg:px-10">
        ...
        <div className="flex items-center gap-2">
          {user ? (
            <>
              <Button asChild variant="ghost" className="h-9 rounded-full px-4 text-sm text-foreground hover:bg-black/5">
                <Link to="/dashboard">Dashboard</Link>
              </Button>
              <Button asChild className="h-9 rounded-full bg-foreground px-4 text-sm font-semibold text-background hover:bg-foreground/90">
...
function Section({
  ...
  return (
    <section id={id} className={`relative z-10 scroll-mt-20 overflow-hidden px-6 py-24 lg:px-10 ${toneClass}`}>
      {tone === "grid" && (
        <div
          aria-hidden
      ...
      >
        ...
        <h2 className={`mt-3 max-w-3xl text-3xl font-semibold tracking-tight sm:text-4xl ${titleClass}`}>
          {title}
        </h2>
        {children}
      </motion.div>
...
const TECH = [
  { eyebrow: "Architectures", title: "Transmon & Superconducting Qubits", desc: "Generate 5-qubit transmon processors up to 128-qubit superconducting arrays with optimized layouts." },
  { eyebrow: "Error correction", title: "Surface & Steane Codes", desc: "Quantum error correction architectures built directly into the generated chip topology." },
  { eyebrow: "Coupling", title: "Nearest-Neighbour & Bus", desc: "Choose coupling networks — nearest-neighbour, all-to-all bus, or custom routing — generated to your prompt." },
] as const;
...
const DEMO_OUTPUTS = [
  { icon: Cpu, title: "Architecture", value: "5-qubit transmon", detail: "Nearest-neighbor 2D coupling" },
  { icon: Terminal, title: "Specifications", value: "T1 ≈ 60μs · T2 ≈ 45μs", detail: "Fidelity target 99.5%" },
